...
export default function ProjectDetailsPage() {
  ...
  const currentEmployee = useMemo(() => {
    if (user && employees.length > 0) {
      return employees.find(
        (e) => e.authUid === user.uid || e.email === user.email,
      );
    }
    return null;
  }, [user, employees]);
  ...
  const [activeSubtaskParentId, setActiveSubtaskParentId] = useState<
    string | null
  >(null);
  ...
  const [resourceSheetView, setResourceSheetView] = useState<
    "list" | "add" | "detail"
  >("list");
  ...
  const isMarketingTask = (item: any) =>
    !!(item.platform || item.week || item.monthYear);
  ...
  useEffect(() => {
    ...
    combinedItems.sort((a, b) => {
      ...
      const aTime = a.id.includes("task-")
        ? parseInt(a.id.split("-")[1])
        : a.id.includes("auto-")
          ? parseInt(a.id.split("-")[1])
          : 0;
      const bTime = b.id.includes("task-")
    ...
    const isTeamMember =
      ...
      ((currentEmployee.team &&
        (project.assignedTeams?.includes(currentEmployee.team) ||
          project.assignedTeam === currentEmployee.team)) ||
        project.devTeam?.some(
          (d: any) =>
            (typeof d === "string" ? d : d.name) === currentEmployee.name,
        ));
    ...
    if (
      ...
      combinedItems = combinedItems.filter((item) => {
        ...
        return assignees.some(
          (a: any) =>
            (typeof a === "string" ? a : a.name) === currentEmployee.name,
        );
      });
    }
  ...
  const mergeIntoFullMilestones = (localItems: any[], deletedId?: string) => {
    const fullMilestones = (project?.milestones || []).filter(
      (m: any) => m.task && m.task.trim().length > 0,
    );
    ...
    const updatedItems = items.map((item) => {
      if (item.id === itemId) {
        ...
        if (isCompleted && item.subtasks && item.subtasks.length > 0) {
          const allSubtasksDone = item.subtasks.every(
            (st: any) => st.completed,
          );
          if (!allSubtasksDone) {
            toast.error(
              "Please complete all subtasks before marking this task as completed.",
            );
            hasError = true;
            return item;
          }
  ...
  const handleHideToggle = async (itemId: string) => {
    ...
    projectService
      ...
      .then(() => {
        const isHidden = updatedItems.find(
          (i) => i.id === itemId,
        )?.isHiddenFromClient;
        toast.success(
          isHidden ? "Task hidden from client" : "Task visible to client",
        );
      })
      .catch((error) => {
  ...
  const handleTaskUpdate = async (updatedTask: any) => {
    ...
    const updatedItems = items.map((item) =>
      item.id === updatedTask.id
        ? { ...updatedTask, lastEditedAt: new Date().toISOString() }
        : item,
    );
  ...
  const confirmDeleteTask = async () => {
    ...
    try {
      const fullMilestones = mergeIntoFullMilestones(
        updatedItems,
        taskToDelete,
      );
      const cleanItems = sanitizeForFirestore(fullMilestones);
  ...
  const handleAddTask = async () => {
    ...
    const newTask = {
      ...
      assignedTo: currentEmployee
        ? { name: currentEmployee.name, avatar: currentEmployee.avatar || "" }
        : project.devTeam?.[0] || { name: "Unassigned", avatar: "" },
      notes: [],
      attachments: [],
    };
  ...
  const handleToggleSubtaskInline = async (
    parentId: string,
    subtaskId: string,
  ) => {
    if (!project?.id) return;
  ...
  const handleDeleteSubtaskInline = async (
    parentId: string,
    subtaskId: string,
  ) => {
    if (!project?.id) return;
  ...
  const handleAddResource = async () => {
    ...
    try {
      ...
      if (resourceType === "file" && resourceFile) {
        content = await projectService.uploadProjectFile(
          project.id,
          resourceFile,
        );
  ...
  const handleDeleteResource = async (resourceId: string) => {
    ...
    try {
      const resourceToDelete = project.resources?.find(
        (r) => r.id === resourceId,
      );
  ...
  const [actionType, setActionType] = useState<
    "reject" | "doubt" | "view-reject" | "view-doubt" | null
  >(null);
  const [actionReason, setActionReason] = useState("");

  const handleClientActionClick = async (
    task: any,
    type: "approve" | "reject" | "doubt" | "view-reject" | "view-doubt",
  ) => {
    if (!project?.id) return;
    ...
    if (type === "approve") {
      ...
      if (lastNote?.text) {
        ...
        const prefix =
          type === "view-reject" ? "REJECTION REASON: " : "CLIENT DOUBT: ";
  ...
  const submitClientAction = async () => {
    ...
    const updatedItems = items.map((item) => {
      if (item.id === actionTask.id) {
        ...
        const timestamp = new Date().toISOString();
        const prefix =
          actionType === "reject" ? "REJECTION REASON" : "CLIENT DOUBT";
    ...
    <div className="flex-1 space-y-4 p-8 pt-6 min-h-screen">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <SheetDescription className="text-slate-500 font-medium">
                    {resourceSheetView === "list" &&
                      "Key items and shared documentation."}
                    {resourceSheetView === "add" &&
                      "Share a new requirement or file."}
                    {resourceSheetView === "detail" &&
                      `Created by ${selectedResource?.createdBy}`}
                  </SheetDescription>
                </div>
              </div>
          ...
          <div className="p-8">
            {resourceSheetView === "add" && (
              <div className="space-y-8 animate-in slide-in-from-right duration-300">
                <Tabs
                  defaultValue="text"
                  onValueChange={(v: any) => setResourceType(v)}
                  className="w-full"
                >
                  ...
                  <div className="mt-8 space-y-6">
                    <div className="space-y-2">
                      <Label className="text-[11px] font-bold uppercase text-slate-500 pl-1">
                        Resource Title
                      </Label>
                      <Input
                          ...
                          {resourceFile ? (
                            <>
                              ...
                              <Button
                                ...
                                className="text-[10px] font-black uppercase text-red-500 hover:bg-red-50 hover:text-red-700"
                              >
                                Remove File
                              </Button>
                            </>
                    ...
                    <div className="pt-4 flex gap-3">
                      ...
                      <Button
                        ...
                        disabled={
                          ...
                          (resourceType === "text" && !resourceContent.trim())
                        }
                      ...
                      >
                        {isSavingResource ? (
                          ...
                          <>
                            <Save className="h-4 w-4 mr-2" /> Share Resource
                          </>
                        )}
                      </Button>
                    </div>
              ...
              <section className="space-y-6 animate-in fade-in duration-300">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-black uppercase tracking-widest text-slate-400">
                    Shared Assets ({project.resources?.length || 0})
                  </Label>
                </div>
                    ...
                    <div className="py-20 flex flex-col items-center justify-center text-center space-y-4 bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-100">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium text-slate-300 max-w-[220px] mx-auto mt-1 leading-relaxed">
                          Add project requirements, wireframes, or credentials
                          for the team.
                        </p>
                      </div>
                      {!isClient && (
                      ...
                      <div className="flex flex-col items-center justify-center py-6 gap-6">
                        ...
                        <div className="text-center">
                          <p className="font-bold text-slate-900 border-b border-slate-200 pb-1 mb-1">
                            {selectedResource.fileName || "Project Document"}
                          </p>
                          <p className="text-xs text-slate-500">
                            Resource is ready for download
                          </p>
                        </div>
                        <Button
                          asChild
                          className="w-full h-12 bg-primary hover:bg-primary/90 rounded-xl font-bold shadow-lg shadow-primary/10"
                        >
                          <a
      ...
      <div className="flex items-center justify-between space-y-2 mb-8 pb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-medium">
                Project Tracker & Timeline • {project.client}
              </p>
            </div>
          </div>
          <Button
            ...
            className="h-8 rounded-full px-3 text-xs font-semibold uppercase tracking-[0.18em]"
          >
            Testing
          </Button>
        </div>
        <div className="flex items-center gap-3">
          <Badge
            variant="outline"
            className="px-3 py-1 border-slate-200 text-slate-600 font-medium"
          >
          ...
          {project.category && (
            <Badge
              variant="secondary"
              className="bg-slate-100 text-slate-600 px-3 py-1.5 font-bold uppercase tracking-wider text-[10px]"
            >
              {project.category}
            </Badge>
          )}
            ...
            <div className="flex">
              ...
              >
                <TabsList className="grid w-full grid-cols-2 p-1 bg-slate-100/50 rounded-xl h-12 border border-slate-200">
                  <TabsTrigger
                    value="marketing"
                    className="rounded-lg font-bold text-[11px] uppercase tracking-wider"
                  >
                    Marketing Calendar
                  </TabsTrigger>
                  <TabsTrigger
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center space-x-2 bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 shadow-sm">
                    <Button
                      ...
                      className="h-8 w-8 rounded-lg hover:bg-white hover:text-slate-900 hover:shadow-sm text-slate-500 transition-all"
                    >
            ...
            <div className="overflow-visible">
              <Table>
                <TableHeader className="bg-gray-50">
                  {project.category === "Digital Marketing" ? (
                    <TableRow>
                      <TableHead className="w-[120px] py-3 whitespace-nowrap text-xs font-bold uppercase tracking-wider text-slate-500">
                        Platform
                      </TableHead>
                      <TableHead className="w-[150px] py-3 whitespace-nowrap text-xs font-bold uppercase tracking-wider text-slate-500">
                        Post ID/Link
                      </TableHead>
                      <TableHead className="py-3 min-w-[200px] whitespace-nowrap text-xs font-bold uppercase tracking-wider text-slate-500">
                        Content Title
                      </TableHead>
                      <TableHead className="w-[110px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Type
                      </TableHead>
                      <TableHead className="w-[120px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Scheduled
                      </TableHead>
                      <TableHead className="w-[120px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Actual
                      </TableHead>
                      <TableHead className="w-[130px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Status
                      </TableHead>
                      <TableHead className="w-[140px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Responsible
                      </TableHead>
                      <TableHead className="w-[130px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Approval
                      ...
                      <TableHead className="w-[150px] py-3 whitespace-nowrap text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                        Files
                      </TableHead>
                      <TableHead className="py-3 min-w-[150px] whitespace-nowrap text-xs font-bold uppercase tracking-wider text-slate-500">
                        Remarks
                      </TableHead>
                      <TableHead className="w-[100px] py-3 text-center whitespace-nowrap text-xs font-bold uppercase tracking-wider text-slate-500">
                        Delivered
                      </TableHead>
                    ...
                    <TableRow>
                      ...
                      <TableHead className="w-[150px] py-3 text-right">
                        Date
                      </TableHead>
                      {!isClient && (
                      ...
                      <TableHead className="w-[120px] py-3 text-center">
                        <div className="flex items-center justify-center gap-2 group/activity">
                          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover/activity:text-slate-600 transition-colors">
                            Activity
                          </span>
                          {!isClient && (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <Plus
                              className={cn(
                                "h-3 w-3",
                                newTaskName ? "text-primary" : "text-slate-300",
                              )}
                            />
                          </div>
                          <Input
                  ...
                  {project.category === "Digital Marketing" ? (
                    <>
                      {(() => {
                        ...
                        const weekItems = items.filter((item) => {
                          ...
                          const itemMonth =
                            item.monthYear || format(new Date(), "MMMM yyyy");
                        ...
                        return (
                          <React.Fragment key={week}>
                            <TableRow className="bg-transparent hover:bg-slate-50/50 group/week border-none">
                              <TableCell
                                colSpan={isClient ? 11 : 14}
                                className="pt-6 pb-4 px-6 border-none"
                              >
                                <div className="flex items-center gap-3">
                                  <span className="font-bold text-blue-800 text-xs uppercase tracking-widest">
                                    {week}
                                  </span>
                                  <Badge
                                    variant="secondary"
                                    className="bg-blue-50 text-blue-600 border-none text-[10px] font-bold px-2 py-0 h-5"
                                  >
                                    {weekItems.length} post
                            ...
                            {weekItems.length === 0 && (
                              <TableRow>
                                <TableCell
                                  colSpan={isClient ? 11 : 14}
                                  className="py-16 px-6 text-center border-b border-slate-50"
                                >
                    ...
                    items.map((item, index) => {
                      ...
                      const showDateHeader =
                        !prevItem || prevItem.date !== item.date;
                      ...
                      const formatDateHeader = (dateStr: string) => {
                        ...
                        try {
                          ...
                          if (isValid(fallbackDate))
                            return format(fallbackDate, "EEEE, MMMM do, yyyy");
                          return dateStr;
                        } catch (e) {
                          return dateStr;
                        }
                        ...
                        <React.Fragment key={`group-${item.id}`}>
                          ...
                          {expandedTasks.has(item.id) &&
                            item.subtasks?.map((st: any) => (
                              <TableRow
                                key={st.id}
                                className="group/subtask hover:bg-slate-50/40 border-none"
                              >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    General tasks, additional requirements or management items.
                  </p>
                </div>
              </div>
              <div className="overflow-visible">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {items.filter((item) => !isMarketingTask(item)).length >
                    0 ? (
                      items
                        .filter((item) => !isMarketingTask(item))
                        .map((item, index) => {
